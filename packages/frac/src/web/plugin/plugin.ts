...
export interface FracPluginOptions {
  ...
  viewsDir?: string;
  /** Directory scanned for Fractal components. Defaults to `"src/fractals"`. */
  fractalsDir?: string;
}
...
function resolveDefaultFractalsDir(_projectRoot: string): string {
  return "src/fractals";
}
...
export function frac(options?: FracPluginOptions): Plugin {
  ...
  const configuredFractalsDir = options?.fractalsDir;
  ...
  return {
    ...
    api: { viewsDir: rawViewsDir, fractalsDir: configuredFractalsDir },
