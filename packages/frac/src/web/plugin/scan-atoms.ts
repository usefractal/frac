...
export function scanAtomsSync(fractalsDir: string): ScannedAtoms {
  const flatPattern = resolve(fractalsDir, "*.{tsx,jsx}");
  const dirPattern = resolve(fractalsDir, "*/index.{tsx,jsx}");
...
export function discoverAtomsSync(fractalsDir: string): DiscoveredAtom[] {
  const { valid } = scanAtomsSync(fractalsDir);
...
export function scanAndWriteAtomsDts(
  ...
  const rawDir = fractalsDir ?? "src/fractals";
