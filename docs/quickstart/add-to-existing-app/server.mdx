---
title: Use frac/server
description: "Add view registration and TypeScript type inference to your existing MCP server with frac/server."
---

# Use frac/server

This guide shows you how to add `frac/server` to an existing MCP server to enable view support with full TypeScript type inference.
...
Your existing tools, resources, and prompts will continue to work as before. With `frac/server`, `registerTool` provides an optional `view` field so you can bind a tool to a React view in a single call.
