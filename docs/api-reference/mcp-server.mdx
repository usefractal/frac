...
```typescript
import { requireBearerAuth } from "frac/server";
...
```typescript
import { optionalBearerAuth } from "frac/server";
...
```typescript
import { type AuthInfo, InvalidTokenError } from "frac/server";
...
**Example:**
```typescript
import { mcpAuthMetadataRouter } from "frac/server";
...
The following middleware types are available from `frac/server`:
